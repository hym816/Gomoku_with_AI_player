...
class GomokuBoard_1 : public QWidget
{
public:
    GomokuBoard_1(QWidget *parent = nullptr) : QWidget(parent)
    {
        for (int i = 0; i < 15; i++)
        {
            for (int j = 0; j < 15; j++)
        ...
        // 连接动作的信号与槽函数
        connect(newGameAction, &QAction::triggered, this, &GomokuBoard_1::resetBoard);
        connect(exitAction, &QAction::triggered, this, &GomokuBoard_1::close);
        ...
        if (event->button() == Qt::LeftButton)
        {
            ...
            if (isValidMove(row, col))
            {
                ...
                else if (currentPlayer == aiPlayer)
                {
                    aiMakeMove(); // 让AI自动下棋
                    // 在上一步AI走棋的位置绘制红点
                    if (lastAIMoveRow >= 0 && lastAIMoveCol >= 0) {
                        QPainter painter(this);
                        painter.setBrush(Qt::red);
                        int centerX = 0 + col * cellSize + cellSize / 2;
                        int centerY = 0 + row * cellSize + cellSize / 2;
                        int radius = cellSize +100;  // 红点的半径
                        painter.drawEllipse(centerX - radius, centerY - radius, 2 * radius, 2 * radius);
                        qDebug() << centerX - radius << centerY - radius;
                    }
                }
            }
        }
...
private:
    ...
    enum { EmptyPiece, WhitePiece, BlackPiece  };

    int board[BoardSize][BoardSize];
    int currentPlayer = BlackPiece;
    int cellSize = CellSize;
    bool gameOver = false;
    int aiPlayer = WhitePiece;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void calculate()
    {
        ...
        board[x][y] = 1;
        QPainter painter(this);
        drawPiece(painter, x, y, Qt::white);
